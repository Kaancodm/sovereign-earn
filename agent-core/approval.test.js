...
const approved = store.approve(approval.approvalId, 'human');
...
assert.throws(() => store.assertUsable({ approvalId: approval.approvalId, request: base }), /replay detected/);
assert.throws(() => store.consume(approval.approvalId, base), /replay detected/);
