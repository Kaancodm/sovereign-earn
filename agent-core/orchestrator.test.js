...
test("unknown agent or skill is rejected", () => {
  ...
  assert.throws(
    () => orchestrator.startRun({ agentId: "unknown", skillId: "repository.read" }),
    /unknown agent or skill/
  );
});
...
test("agent cannot execute a skill it is not allowed to use", () => {
  const orchestrator = new Orchestrator({ policyRules: RULES });
  assert.throws(
    () => orchestrator.startRun({ agentId: "headcoder", skillId: "account.outreach" }),
    /not allowed/
  );
});
...
test("allowed tool access returns allow", () => {
  ...
  const result = orchestrator.authorizeTool({
    runId: run.runId,
    agentId: "headcoder",
    skillId: "repository.read",
    capability: "github",
...
test("approval-required tool stays blocked until approval", () => {
  const orchestrator = new Orchestrator({ policyRules: RULES });
  const run = orchestrator.startRun({ agentId: "partnership", skillId: "account.outreach" });
  const pending = orchestrator.authorizeTool({
    runId: run.runId,
    agentId: "partnership",
    skillId: "account.outreach",
    capability: "mail",
  ...
  const approved = orchestrator.authorizeTool({
    runId: run.runId,
    agentId: "partnership",
    skillId: "account.outreach",
    capability: "mail",
