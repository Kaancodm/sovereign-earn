...
const runtimeContext = (overrides = {}) => {
  ...
  const skillId = overrides.skillId || 'repository-analysis';
  return {
    runId,
    agentId,
    budget: createBudget(),
    governanceContext: createGovernanceContext({ identity: { userId: 'user-1', role: 'user' }, sessionMode, scopes: ['github.read'] }),
    contextManifest: { contextVersion: '1.0.0', runId, agentId, sessionMode, architectureVersion: '1.0.0', policyVersion: '1.0.0', flowVersion: '1.0.0', workspaceMapVersion: '1.0.0', skillVersions: [`${skillId}@1.0.0`], artifactRefs: [], dataClassification: { prompt: 'model_visible', userRecord: 'core_only' } },
...
test('handoff cannot activate privileged capability outside the target policy scope', async () => { setup(); registerAgent({ id: 'handoff-agent', active: true }); registerSkill({ id: 'handoff-repository-analysis', allowedAgents: ['handoff-agent'] }); const counter = { calls: 0 }; registerTool({ skillId: 'handoff-repository-analysis', capability: 'github.read', action: 'readRepo', execute: async () => { counter.calls += 1; } }); const targetRequest = { ...request(), runId: 'run-2', toolCallId: 'tool-2', agentId: 'handoff-agent', skillId: 'handoff-repository-analysis' }; const targetRuntime = new ToolRuntime({ policyRules: [{ agentId: 'handoff-agent', skillId: 'handoff-repository-analysis', capability: 'github.read', action: 'readRepo', decision: 'deny', privileged: true }], approvalStore: new ApprovalStore(), audit: new AuditLog(), runtimeContext: runtimeContext({ runId: 'run-2', agentId: 'handoff-agent', skillId: 'handoff-repository-analysis' }) }); const result = await targetRuntime.executeTool(targetRequest); assert.equal(result.status, 'blocked'); assert.equal(result.decision.category, 'policy_violation'); assert.equal(counter.calls, 0); });
